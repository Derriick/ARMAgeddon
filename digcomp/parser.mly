...
code:
  ...
  | ADD REG COMA REG COMA REG COMA INT  { assert (0<=$8 && $8<=3); Add  ($2,$4,$6,false,$8) }
  ...
  | SUB REG COMA REG COMA REG COMA INT  { assert (0<=$8 && $8<=3); Add  ($2,$4,$6,true,$8) }
  ...
  | CMP REG COMA REG COMA INT           { assert (0<=$6 && $6<=3); Cmp ($2, $4, $6)     }
  | CMP REG COMA REG                    { Cmp ($2, $4, 0)    }
  | LD REG COMA REG COMA INT            { assert (0<=$6 && $6<=3); Load ($2, $4, $6)    }
  | LD REG COMA REG                     { Load ($2, $4, 0)   }
  | MOV REG COMA LPAR REG RPAR COMA INT { assert (0<=$8 && $8<=3); Load ($2, $5, $8)    }
  | MOV REG COMA LPAR REG RPAR          { Load ($2, $5, 0)   }
  | ST REG COMA REG COMA INT            { Store ($4, $2, $6) }
  | ST REG COMA REG                     { Store ($4, $2, 0)  }
  | MOV LPAR REG RPAR COMA REG COMA INT { assert (0<=$8 && $8<=3); Store ($3, $6, $8) }
